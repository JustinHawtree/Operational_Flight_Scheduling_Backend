import generate_runner from "../../generation/gen";

const generationHandler = async (action: string, payload: { [key: string]: string }, callback: (error: any, response: any) => any) => {
  switch (action) {
    ...
    case "generate":
      ...
      try {
        let flights: Array<any> = await generate_runner(payload);
        console.log("Flights ready to send!");
        console.log("Flights:", flights);
        callback(false, { flights });
        // callback(false, 
        //   { 
        //     flights: [
        //       {
